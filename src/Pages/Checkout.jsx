...
const Checkout = () => {
  ...
  const handlePlaceOrder = async (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      try {
        const { status, shipping_address, shipping_city, postal_code, shipping_country } = billingData;
        ...
        setOrderedProducts(orderedProducts.filter(p => p)); // Set ordered products state
        setTotalOrderAmount(getTotalCartAmount()); // Set total order amount
        setButtonPopup(true); // Show the popup on success
